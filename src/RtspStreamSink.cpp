...
void RtspStreamSink::on_klv_pad_added (GstElement * element, GstPad * srcpad, gpointer data)
{
  ...
  // only attach if src is a video stream
...
void RtspStreamSink::on_h265_pad_added (GstElement * element, GstPad * srcpad, gpointer data)
{
  ...
  if( !strncmp(name, "video/x-h265", 12) )  {
    ...
    if( streamsink->getParserPtr() == NULL ) {
      ...
      if (!gst_element_link_many( streamsink->getParserQueuePtr(), streamsink->getParserPtr(), dec265, enc264, streamsink->getHlsQueuePtr(), NULL)) {
...
void RtspStreamSink::on_h264_pad_added (GstElement * element, GstPad * srcpad, gpointer data)
{
  ...
  if( !strncmp(name, "video/x-h264", 12) )  {
    ...
    if( streamsink->getParserPtr() == NULL ) {
      ...
      if (!gst_element_link_many( streamsink->getParserQueuePtr(), streamsink->getParserPtr(), streamsink->getHlsQueuePtr(), NULL)) {
...
void RtspStreamSink::upload() {
   ...
   uploadCommand << "python3 ./scripts/fingerprint.py " << _info.name << "_" << _datetimeStr  << " " << _archivedir << " " << _currentArchDir << " " << _uploadUrl << " " << _epochStart << " " << _epochEnd << " &";
...
void RtspStreamSink::run( RtspStreamSink& self) {
  ...
  do {
    ...
    do{  
      ...
      GstMessage* msg = gst_bus_timed_pop_filtered (bus, GST_CLOCK_TIME_NONE, (GstMessageType) ( GST_MESSAGE_ELEMENT | GST_MESSAGE_ERROR | GST_MESSAGE_EOS));
      if (msg != NULL) {
        GError *err;
        gchar *debug_info;
...
RtspStreamSink::RtspStreamSink( StreamInfo info, std::string archivedir, std::string hlsdir, std::string uploadurl, std::string portrange ) :
